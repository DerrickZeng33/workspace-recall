...
Space Recorder is designed to work locally. It has no built-in network
...
or folder paths. Space Recorder does not save the complete command line.
...
Space Recorder protects this directory with Windows access rules for the
...
a fresh capture request from Space Recorder; it does not continuously record
the active document.
...
Close Space Recorder, disable the Revit integration if it is enabled, and
